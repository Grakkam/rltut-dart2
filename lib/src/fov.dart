...
class Fov {
  ...
  void refresh(Vec pos) {
    for (var pos in _gameMap.bounds) {
      _gameMap.visible[pos] = false;
    }
    ...
    for (var row = 1; row < maxRows; row++) {
      ...
      for (var col = 0; col <= row; col++) {
        ...
        if (fullShadow) {
          ...
          _gameMap.visible[pos] = visible;

          // Add any opaque tiles to the shadow map.
          if (visible && _gameMap.tiles[pos] == TileTypes.wall) {
            line.add(projection);
  ...
  Shadow _projectTile(int row, int col) {
    ...
    return Shadow(
        topLeft, bottomRight, Vec(col, row + 2), Vec(col + 1, row + 1));
  }
}
